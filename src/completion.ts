...
const rules: Record<string, Rule> = {
  ...
  agent: {
    ...
    flags: ['--profile', '--mode', '--host', '--port', '--agent-id', '--service', '--provider-runtime', '--provider-executor', '--provider-executor-options', '--provider-url', '--name', '--description', '--price', '--available-tokens', '--capabilities', '--capabilities-file', '--public', '--delisted', '--auth', '--capability', '--output-mode', '--context', '--show-plan', '--gateway-base-url', '--local-base-url', '--openclaw-bin', '--standard-plugin-source', '--official-a2a-plugin-source', '--skip-standard-plugin', '--upstream-path', '--protected-path', '--provider-path', '--agent-card-path', '--market-path', '--path', '--paths', '--card-path', '--service-id', '--kind', '--hostname', '--https-port', '--include-standard', '--disable', '--dry-run', '--allow-unauthenticated', '--forward-authorization', '--restart', '--json'],
    subcommandFlags: {
      profile: ['--name', '--description', '--host', '--port', '--agent-id', '--service', '--provider-runtime', '--provider-executor', '--provider-executor-options', '--provider-url', '--price', '--available-tokens', '--capabilities', '--capabilities-file'],
      publish: ['--profile', '--name', '--description', '--service', '--provider-runtime', '--provider-executor', '--provider-executor-options', '--provider-url', '--price', '--available-tokens', '--capabilities', '--capabilities-file', '--json'],
      'update-listing': ['--profile', '--agent-id', '--public', '--delisted', '--name', '--description', '--service', '--provider-runtime', '--provider-executor', '--provider-executor-options', '--provider-url', '--price', '--available-tokens', '--capabilities', '--capabilities-file', '--json'],
      serve: ['--profile', '--mode', '--host', '--port', '--agent-id', '--service', '--provider-runtime', '--provider-executor', '--provider-executor-options', '--provider-url'],
      connect: ['--profile', '--agent-id', '--service', '--provider-runtime', '--provider-executor', '--provider-executor-options', '--provider-url'],
